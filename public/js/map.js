...
let projectPopupsEnabled = false;
...
const coHolidayCache = {};

function colombiaHolidaySet(year) {
  if (coHolidayCache[year]) return coHolidayCache[year];
  const set = new Set();
  ...
  fijo.forEach((d) => set.add(dateISO(d)));
  const trasladados = [
    new Date(year, 0, 6),
    new Date(year, 2, 19),
    new Date(year, 5, 29),
  ...
  trasladados.forEach((d) => set.add(dateISO(moveToNextMonday(d))));
  const juevesSanto = addDays(e, -3);
  const viernesSanto = addDays(e, -2);
  set.add(dateISO(juevesSanto));
  set.add(dateISO(viernesSanto));
  const ascension = moveToNextMonday(addDays(e, 40));
  const corpus = moveToNextMonday(addDays(e, 60));
  const sagrado = moveToNextMonday(addDays(e, 68));
  set.add(dateISO(ascension));
  set.add(dateISO(corpus));
  set.add(dateISO(sagrado));
  coHolidayCache[year] = set;
  return set;
}

function isColombiaHoliday(d) {
  const y = d.getFullYear();
  const set = colombiaHolidaySet(y);
  return set.has(dateISO(d));
}

function isBusinessDayCO(d) {
  return d.getDay() !== 0 && !isColombiaHoliday(d);
}

function businessDaysBetweenCO(startDate, endDate) {
  if (!startDate || !endDate) return 0;
  const s = new Date(
    startDate.getFullYear(),
    startDate.getMonth(),
...
function computeBounceClass(props) {
  const motivo = String(props["Motivo"] || "")
    .trim()
    .toUpperCase();
  const reqDate = parseDDMMYYYY(props["Fecha Solicitud"]);
  if (!reqDate) return "";
  const days = businessDaysBetweenCO(reqDate, new Date());
  if (motivo === "CANCELO FACTURA") {
    if (days === 0) return "pin-bounce-slow";
    if (days >= 1) return "pin-bounce-fast";
    return "";
  }
  if (motivo === "RECLAMACION CONSUMOS") {
    if (days === 3) return "pin-bounce-slow";
    if (days >= 4) return "pin-bounce-fast";
    return "";
  }

  if (motivo === "DEUDA PENDIENTE") {
    if (days === 2) return "pin-bounce-slow";
    if (days === 3) return "pin-bounce-fast";
    return "";
  }

    if (motivo === "A SOLICITUD DE USUARIO") {
    if (days === 2) return "pin-bounce-slow";
    if (days === 3) return "pin-bounce-fast";
    return "";
  }

  //DEUDA PENDIENTE A SOLICITUD DE USUARIO
  return days >= 3 ? "pin-bounce-fast" : days >= 2 ? "pin-bounce-slow" : "";
}
  ...
  if (files.length === 0) {
    ...
    files.forEach((file) => {
      ...
      deleteBtn.addEventListener("click", async () => {
        ...
        if (!confirmed) return;
        try {
          const resp = await fetch("/files/" + encodeURIComponent(file), {
            method: "DELETE",
          });
...
async function saveColoredExcel() {
  ...
  try {
    const resp = await fetch(
      "/files/" + encodeURIComponent(file) + "/colorized",
      {
        method: "POST",
...
function openSummaryColorPicker(target) {
  ...
  Array.from(grid.querySelectorAll(".preset-color")).forEach((el) => {
    el.classList.toggle(
      "selected",
      el.dataset.color.toLowerCase() === input.value.toLowerCase()
    );
    el.onclick = () => {
  ...
  input.onchange = function () {
    Array.from(grid.querySelectorAll(".preset-color")).forEach((x) => {
      x.classList.toggle(
        "selected",
        x.dataset.color.toLowerCase() === input.value.toLowerCase()
      );
    });
  };
  modal.classList.add("show");
...
function applySummaryColor() {
  const input = document.getElementById("colorPickerInput");
  if (
    typeof correriaColorPickerActive !== "undefined" &&
    correriaColorPickerActive
  ) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  customColorPicker.addEventListener("change", function () {
    ...
    const matchingOption = Array.from(colorOptions).find(
      (opt) => opt.dataset.color.toLowerCase() === selectedColor.toLowerCase()
    );
    if (matchingOption) {
...
function openWorkshopModal() {
  if (!currentProjectFile) {
    Swal.fire(
      "Proyecto requerido",
      'Abre un proyecto en "Proyectos guardados" para gestionar correrías',
      "warning"
    );
    return;
  }
...
async function saveCorreriaName() {
  if (hasUnsavedCorreriaChanges()) {
    await Swal.fire(
      "Guardar requerido",
      "Guarde los cambios recientes de la correría antes de crear una nueva",
      "warning"
    );
    return;
  }
  ...
  if (!name) {
    await Swal.fire(
      "Dato requerido",
      "Ingrese el nombre de la correría",
      "warning"
    );
    return;
  }
  ...
  try {
    if (currentProjectFile) {
      const resp = await fetch(
        "/projects/" + encodeURIComponent(currentProjectFile) + "/correrias"
      );
      const data = await resp.json();
      if (resp.ok) {
        const existsServer = (data.correrias || []).some(
          (it) =>
        ...
        if (existsServer) {
          await Swal.fire(
            "Ya existe",
            "Ya existe una correría con ese nombre",
            "warning"
          );
          return;
        }
  ...
  const existsLocal = (correrias || []).some(
    (c) =>
      String(c.name || "")
        .trim()
        .toUpperCase() === norm
...
function hasUnsavedCorreriaChanges() {
  ...
  for (const row of rows) {
    ...
    const saveBtn = saveCell
      ? saveCell.querySelector("button.icon-btn.save")
      : null;
...
async function loadProjectCorrerias() {
  try {
    ...
    const data = await resp.json();
    if (!resp.ok)
      throw new Error(data.error || "No se pudieron leer correrías");
...
function renderCorreriasFromProject(list) {
  ...
  list.forEach((item) => {
    ...
    correrias.push({
      ...
      estado: item.estado,
      color: item.color,
      estadoCorreria: item.estadoCorreria || "",
    });
    const tr = document.createElement("tr");
    const tdName = document.createElement("td");
...
function plotSelectedCorrerias() {
  if (!currentProjectFile) {
    Swal.fire(
      "Proyecto requerido",
      'Abre un proyecto en "Proyectos guardados"',
      "warning"
    );
    return;
  }
  const table = document.getElementById("correriaTable");
  const checks = table
    ? table.querySelectorAll("tbody input.correria-select:checked")
    : [];
  const descs = Array.from(checks)
    .map((cb) => cb.dataset.desc)
    .filter(Boolean);
  if (!descs.length) {
    Swal.fire(
      "Selección requerida",
      "Marca al menos una correría para graficar",
      "info"
    );
    return;
  }
...
function saveCorreria(idx) {
  ...
  if (!currentProjectFile) {
    Swal.fire(
      "Proyecto requerido",
      "Abre un proyecto para guardar Tpl en el Excel",
      "warning"
    );
    return;
  }
  fetch(
    "/projects/" +
      encodeURIComponent(currentProjectFile) +
      "/update-tpl-by-description",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    .catch(() =>
      Swal.fire("Error", "Falló la actualización en Excel", "error")
    );
}
...
function updateCorreriasTotalsBanner() {
  ...
  const count = (correrias || []).filter(
    (c) => (c.estadoCorreria || "").toLowerCase() === "guardado"
  ).length;
...
async function refreshCorreriasTotalsFromServer(file) {
  try {
    const resp = await fetch(
      "/projects/" + encodeURIComponent(file) + "/correrias"
    );
    const data = await resp.json();
    if (!resp.ok)
      throw new Error(data.error || "No se pudieron leer correrías");
    ...
    const count = list.filter(
      (it) => (it.estadoCorreria || "").toLowerCase() === "guardado"
    ).length;
    const el = ensureCorreriasTotalsBanner();
    const valEl = el.querySelector(".correrias-total-value");
    if (valEl) valEl.textContent = String(count);
...
function ensurePendingProgramBanner() {
  let el = document.getElementById("pendingProgramBanner");
  if (!el) {
    el = document.createElement("div");
    el.id = "pendingProgramBanner";
    el.className = "pending-program-banner";
    const label = document.createElement("span");
    label.className = "pending-program-label";
    label.textContent = "Faltan por programar:";
    const value = document.createElement("span");
    value.className = "pending-program-value";
    el.appendChild(label);
    el.appendChild(document.createTextNode(" "));
    el.appendChild(value);
    document.body.appendChild(el);
...
function showPendingProgramBanner() {
  const el = ensurePendingProgramBanner();
  el.style.display = "flex";
}

function hidePendingProgramBanner() {
  const el = document.getElementById("pendingProgramBanner");
  if (el) el.style.display = "none";
}

function updatePendingProgramBanner(val) {
  const el = ensurePendingProgramBanner();
  const vEl = el.querySelector(".pending-program-value");
  if (vEl) vEl.textContent = String(val);
  el.style.display = "flex";
}

async function refreshPendingProgramFromServer(file) {
  try {
    if (!file) return;
    const resp = await fetch(
      "/projects/" + encodeURIComponent(file) + "/pending-programming-count"
    );
    const data = await resp.json();
    if (!resp.ok) throw new Error(data.error || "No se pudo contar pendientes");
    updatePendingProgramBanner(data.pending || 0);
  } catch (e) {
    const el = document.getElementById("pendingProgramBanner");
    if (el) el.style.display = "none";
  }
...
function startCorreriaAssignment(idx) {
  if (!currentProjectFile) {
    Swal.fire(
      "Proyecto requerido",
      'Abre un proyecto en "Proyectos guardados" para asignar puntos',
      "warning"
    );
    return;
  }
  if (projectPopupsEnabled) {
    Swal.fire(
      "Modo vista activo",
      "Desactive 'Mostrar contenedores de puntos' para programar",
      "info"
    );
    return;
  }
  ...
  const isSavedState =
    correrias[idx] &&
    (correrias[idx].estadoCorreria || "").toLowerCase() === "guardado";
  if (isSavedState && !saveEnabled) {
    Swal.fire(
      "Correría guardada",
      "No se puede asignar más puntos a esta correría",
      "info"
    );
    return;
  }
  ...
  if (!name) {
    Swal.fire(
      "Dato requerido",
      "El nombre de la correría es necesario",
      "warning"
    );
    return;
  }
  ...
  fetch(
    "/projects/" + encodeURIComponent(currentProjectFile) + "/ensure-columns",
    { method: "POST" }
  )
    .then((resp) => resp.json())
    .then(() =>
      Swal.fire(
        "Modo asignación",
        "Haga clic en los puntos del mapa para asignarlos a: " + name,
        "info"
      )
    )
    .catch(() =>
      Swal.fire(
        "Advertencia",
        "No se pudieron asegurar columnas, intentará de todas formas",
        "warning"
      )
    );
...
function assignMarkerToCorreria(props, marker, bounceClass) {
  ...
  const saveBtnEl = row
    ? row.children[4]?.querySelector("button.icon-btn.save")
    : null;
  const saveEnabled = !!(saveBtnEl && !saveBtnEl.disabled);
  const isSavedState =
    correrias[ci] &&
    (correrias[ci].estadoCorreria || "").toLowerCase() === "guardado";
  if (isSavedState && !saveEnabled) {
    correriaAssign.active = false;
    hideCorreriaBanner();
    setProjectPopupsEnabled(true);
  ...
  fetch(
    "/projects/" + encodeURIComponent(currentProjectFile) + "/update-record",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    .then(({ ok }) => {
      if (ok) {
        ...
        if (row) {
          correrias[correriaAssign.idx].count =
            (correrias[correriaAssign.idx].count || 0) + 1;
          const tdCount = row.children[1];
          if (tdCount)
            tdCount.textContent = String(correrias[correriaAssign.idx].count);
          updateCorreriaBanner();
          refreshPendingProgramFromServer(currentProjectFile);
        }
      } else {
...
function updateCorreriaBanner() {
  ...
  const idx =
    correriaAssign && typeof correriaAssign.idx === "number"
      ? correriaAssign.idx
      : 0;
...
function setProjectPopupsEnabled(enabled) {
  ...
  try {
    ...
    if (!enabled && typeof map !== "undefined" && map && map.closePopup)
      map.closePopup();
  } catch (e) {
...
function initContextMenu() {
    ...
    if (toggleCb.checked) {
      ...
      if (table) {
        Array.from(table.querySelectorAll("tbody tr")).forEach((row) => {
          const btn = row.children[6]?.querySelector("button.icon-btn.add");
        ...
        Array.from(table.querySelectorAll("tbody tr")).forEach((row, idx) => {
          ...
          if (addBtn) {
            ...
            const isSavedState =
              correrias[idx] &&
              (correrias[idx].estadoCorreria || "").toLowerCase() === "guardado";
            addBtn.disabled = isSavedState && !saveEnabled ? true : false;
          }
        });
      }
    }
  ...
  unassignCb.addEventListener("change", () => {
    ...
    if (unassignMode) {
      ...
      Swal.fire(
        "Modo desprogramar",
        "Haga clic en puntos para vaciar campos y pintar con Color Tarea",
        "info"
      );
    } else {
      setProjectPopupsEnabled(true);
  ...
  motivoCb.checked = !!(
    ...
    document.getElementById("motivoModal").classList.contains("show")
  );
  motivoCb.addEventListener("change", () => {
    if (motivoCb.checked) {
      if (!currentProjectFile) {
        Swal.fire(
          "Proyecto requerido",
          "Abre un proyecto para ver el resumen por motivo",
          "warning"
        );
...
function desprogramarMarker(props, marker, bounceClass) {
  ...
  fetch(
    "/projects/" + encodeURIComponent(currentProjectFile) + "/unassign-record",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    .then(({ ok }) => {
      ...
      const colorTarea =
        marker._colorTarea ||
        (marker._props ? marker._props["Color Tarea"] : null) ||
        "#ff4500";
...
function lassoMove(e) {
  ...
  const ll =
    e.latlng ||
    (e.originalEvent ? map.mouseEventToLatLng(e.originalEvent) : null);
  if (!ll) return;
  lassoPath.push(ll);
...
function enableWorkshopDrag(enable) {
  ...
  if (enable) {
    ...
    // touch support
    header.addEventListener("touchstart", workshopDragStart, {
      passive: false,
    });
    document.addEventListener("touchmove", workshopDragMove, {
      passive: false,
    });
  ...
  if (!files || files.length === 0) {
    ...
    files.forEach((file) => {
      ...
      delBtn.addEventListener("click", async () => {
        ...
        if (res.isConfirmed) {
          try {
            const r = await fetch("/projects/" + encodeURIComponent(file), {
              method: "DELETE",
            });
...
async function openProject(file, correriaDesc) {
  try {
    ...
    (data.points || []).forEach((p) => {
      const color =
        typeof p.color === "string" && p.color.trim() ? p.color : "#202020";
      const f = p.props || {};
      const colorCorr =
        typeof f["Color Correria"] === "string" && f["Color Correria"].trim()
          ? f["Color Correria"].trim()
          : "";
      const colorMotivo =
        typeof f["Color Motivo"] === "string" && f["Color Motivo"].trim()
          ? f["Color Motivo"].trim()
          : "";
      const colorTarea =
        typeof f["Color Tarea"] === "string" && f["Color Tarea"].trim()
          ? f["Color Tarea"].trim()
          : "";
      const chosenColor = colorCorr || colorMotivo || colorTarea || color;
      const bounceClass = computeBounceClass(f);
      const marker = L.marker([p.lat, p.lng], {
        icon: createPinIcon(chosenColor, bounceClass),
      });
      const keys = [
        "Cliente",
        "Ruta",
      ...
      const lines = keys
        .map((k) => `<div><strong>${k}:</strong> ${f[k] || ""}</div>`)
        .join("");
    ...
    const asignados = (data.points || []).filter(
      (p) => p.props && p.props["__asignado"] === "1"
    ).length;
    const corrText = Array.isArray(correriaDesc)
      ? correriaDesc.join(", ")
      : correriaDesc || "";
    Swal.fire(
      "Cargado",
      "Se graficaron " +
        total +
        " puntos" +
        (corrText ? " de la correría(s): " + corrText : "") +
        " • Asignados: " +
        asignados,
      "success"
    );
    await refreshCorreriasTotalsFromServer(file);
    await refreshPendingProgramFromServer(file);
    if (!correriaDesc) {
    ...
    Swal.fire(
      "Error",
      e.message || "No se pudieron graficar los puntos",
      "error"
    );
  }
}
...
async function syncClearAndOpen() {
  ...
  const res = await Swal.fire({
    title: "Aviso",
    text: "Guarde los cambios recientes antes de limpiar. ¿Continuar?",
    icon: "warning",
    showCancelButton: true,
...
async function syncMergeAndOpen() {
  ...
  try {
    const resp = await fetch("/projects/merge", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        target: currentProjectFile,
        source: pendingOpenFile,
      }),
    });
    const data = await resp.json();
...
async function loadMotivoSummary(file) {
  try {
    const resp = await fetch(
      "/projects/" + encodeURIComponent(file) + "/motivo-summary"
    );
    const data = await resp.json();
...
function renderMotivoSummaryTable(items) {
  ...
  thM.textContent = "Motivo";
  const thC = document.createElement("th");
  thC.textContent = "Cant.";
  const thP = document.createElement("th");
  thP.textContent = "Prog";
  const thPend = document.createElement("th");
  thPend.textContent = "Pte";
  const thCol = document.createElement("th");
  thCol.textContent = "Color";
  hr.appendChild(thM);
  hr.appendChild(thC);
  hr.appendChild(thP);
  hr.appendChild(thPend);
  hr.appendChild(thCol);
  thead.appendChild(hr);
  table.appendChild(thead);
  const tbody = document.createElement("tbody");
  items.forEach((it) => {
    ...
    tdM.textContent = it.motivo || "";
    const tdC = document.createElement("td");
    tdC.textContent = String(it.count || 0);
    const tdP = document.createElement("td");
    tdP.textContent = String(it.programadas || 0);
    const tdPend = document.createElement("td");
    tdPend.textContent = String(
      Math.max(0, (it.count || 0) - (it.programadas || 0))
    );
    const tdCol = document.createElement("td");
    const swatch = document.createElement("span");
    swatch.className = "color-swatch";
    swatch.style.backgroundColor = it.color || "#000000";
    const label = document.createElement("span");
    label.textContent = it.color || "";
...
function downloadDocType() {
  ...
  if (!file) {
    Swal.fire(
      "Archivo requerido",
      "No se encontró archivo para descargar",
      "warning"
    );
    return;
  }
...
function downloadDocTypeSac() {
  ...
  window.location.href =
    "/projects/" + encodeURIComponent(file) + "/download-sac";
}
...
function downloadDocTypeCorreria() {
  ...
  window.location.href =
    "/projects/" + encodeURIComponent(file) + "/download-correria";
}
